...
- (void)updateUI {
    ...
    [self updateResultLabel:self.resultLabel
                 withObject:[self formatResult:self.game.result
                                      forCards:self.game.cardsFromResult]];
    self.resultLabel.alpha = 1;
    self.flipHistorySlider.maximumValue = self.flipHistory.count;
...
- (void)toggleFlipHistorySlider:(BOOL)enabled {
    self.flipHistorySlider.enabled = enabled;
    self.flipHistorySlider.alpha = enabled ? 1 : .3;
}
...
- (IBAction)browseFlipHistory:(UISlider *)sender {
    ...
    if (selectedHistoryIndex == sender.maximumValue) {
        ...
        self.resultLabel.alpha = .3;
        [self updateResultLabel:self.resultLabel withObject:self.flipHistory[selectedHistoryIndex]];
    }
}
@end
